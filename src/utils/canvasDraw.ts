...
export function getRevealLayout(W: number, H: number, is169: boolean, lineCount: number) {
    const iW = is169 ? W * 0.38 : W * 0.72;
    const iH = is169 ? H * 0.52 : H * 0.34;
...
export function drawFirstFrame(
    ...
    const ir = window.__profileImg_ref__;
    const imgW = is169 ? W * 0.32 : W * 0.65;
    const imgH = is169 ? H * 0.50 : H * 0.32;
    const imgX = W / 2 - imgW / 2;
    const imgY = H / 2 - imgH / 2 - (is169 ? H * 0.08 : H * 0.06);
    ...
    if (ir) {
        ...
        ctx.clip();
        ctx.filter = 'blur(14px) brightness(0.65)'; // ✅ no more as any
        ctx.drawImage(ir, imgX, imgY, imgW, imgH);
        ctx.filter = 'none';                         // ✅ no more as any
        ctx.restore();
    }
    ...
    const titleY = imgY + imgH + (is169 ? H * 0.07 : H * 0.05);
    ctx.save();
...
export function drawSuspense(
    ...
    const cardW = is169 ? W * 0.38 : W * 0.78;
    const cardH = is169 ? H * 0.65 : H * 0.52;
    ...
    ctx.clip();
    const imgEl = window.__profileImg_ref__;
    if (imgEl) {
        drawBlurredImage(ctx, imgEl, cardX + 2, cardY + 2, cardW - 4, cardH - 4, 18);
    } else {
...
export function drawReveal(
    ...
    if (ir) {
        ...
        ctx.translate(-cx, -layout.positions[0]);
        ctx.beginPath();
        ctx.roundRect(iX, iY, layout.iW, layout.iH, 16);
        ctx.clip();
        ctx.shadowColor = GOLD;
        ctx.shadowBlur = 30 * rp;
        drawSharpImage(ctx, ir, iX, iY, layout.iW, layout.iH, lerp(15, 0, easeOutExpo(rp)));
        ctx.restore();
    }
...
export function drawThatsMe(
    ...
    if (ir) {
        ...
        ctx.shadowColor = GOLD;
        ctx.shadowBlur = 20;
        drawSharpImage(ctx, ir, iX, iY, layout.iW, layout.iH, 0);
        ctx.restore();
    }
...
export function drawSocialCards(
    ...
    const visibleCount = Math.max(1, platforms.filter((_, i) => t >= T0 + platforms[i].delay).length);
    ...
    if (is169) {
        ...
        platforms.forEach(({ name, handle, iconFn, delay }, i) => {
...
export function drawFinalCTA(
    ...
    fadeIn(ctx, ip * 0.85, () => {
        ['@overthinkerimam', '@OverthinkerImam', '@Overthinker Imam'].forEach((h, i) => {
            const ix = cx + (i - 1) * iGapSpread;
